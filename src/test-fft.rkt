       ...
       (define (while)
         ...
         (if ((lambda (x y) (not (equal? x y))) span 0)
           (begin
             (begin
               (for
                ((odd (in-range span 2 1)))
                (begin
                  (set! odd (bitwise-ior odd span))
                  (set! even (bitwise-xor odd span))
                  (set! temp
                    (+ (v-list-get real_out even) (v-list-get real_out odd)))
                  (v-list-set!
                   real_out
                   odd
                   (- (v-list-get real_out even) (v-list-get real_out odd)))
                  ...
                  (set! temp
                    (+ (v-list-get img_out even) (v-list-get img_out odd)))
                  (v-list-set!
                   img_out
                   odd
                   (- (v-list-get img_out even) (v-list-get img_out odd)))
...
(define real-in (v-list 1 1 1 1))
(define img-in (v-list 0 0 0 0))
