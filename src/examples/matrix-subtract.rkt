...
; Generate program sketch for matrix sub
(define (matrix-sub-shuffle-sketch mat-A mat-B iterations)
      ...
      (apply choose*
        (map (lambda (out-reg)
          (list
            ...
            (vec-app 'out 'vec-sub (list 'reg-A 'reg-B))
            (vec-write out-reg 'out)))
        C-reg-ids)))

    (append input-shuffles output-sub))
...
; Run a matrix sub sketch with symbolic inputs. If input matrices
; are missing, interp will generate fresh symbolic values.
(define (run-matrix-sub-sketch sketch
                               C-size
                               cost-fn
                               [mat-A #f]
                               [mat-B #f])
  ...
  (define-values (_ cost)
    (interp sketch
            ...
            #:symbolic? #t
            #:cost-fn cost-fn
            #:fn-map (hash 'vec-sub vector-sub)))
...
(define (get-statistics C-size sol reg-of)
  (let*
     ([regs-cost (last (run-matrix-sub-sketch
                      sol
                      C-size
                      (make-register-cost reg-of)))]
     [class-uniq-cost (last (run-matrix-sub-sketch
                            sol
                            C-size
...
(define (matrix-sub:run-experiment spec file-writer)
  ...
  (assert (equal? A-rows B-rows)
          "matrix-sub:run-experiment: Invalid matrix sizes. A-rows not equal to B-rows")
  ...
  (parameterize [(current-reg-size reg-size)]
    ...
    (define msub (matrix-sub-shuffle-sketch A B iterations))
    ...
    (define (sketch-func args)
      (apply (curry run-matrix-sub-sketch
                    msub
                    C-size
                    (cost-fn))
             args))
