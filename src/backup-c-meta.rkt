...
(define (mem assoc-lst key) 
    (let ([found #f])
        (begin
            (for/list ([pair assoc-lst])
                (if (equal? key (cdr pair)) (set! found #t)))
            found)))

(define (get assoc-lst key)
    (let ([val '()])
        (begin
            (for/list ([pair assoc-lst])
                (if (equal? key (cdr pair)) (set! val (cdr pair))))
            val)))

(define (remove assoc-lst key)
    (filter (lambda (pair) (not (equal? key (cdr pair)))) assoc-lst))

(define (insert assoc-lst key val)
    (let ([new-assoc-lst (remove assoc-lst key)])
        (cons (list key val) new-assoc-lst)))

(define array-type-context '())
