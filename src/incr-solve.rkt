...
(require (only-in rosette/solver/solver solver-constructor)
         ...
         (only-in rosette/query/finitize finitize unfinitize)
         (prefix-in $ racket)
         rackunit)
...
(define-syntax synth
  (syntax-rules (synth)
    [(_ #:forall inputs #:assume pre #:guarantee post rest ...)
     (synth-solve-with-finitize (symbolics inputs)
...
(define (synth-solve-with-finitize xs pre post)
  (if (current-bitwidth)
    (begin
      (define fmap (finitize (append pre post)))
      (synth-solve xs
                   (map (curry hash-ref fmap) pre)
                   (map (curry hash-ref fmap) post)
                   fmap))
    (synth-solve xs pre post (make-hash))))

(define (synth-solve xs pre post init-fmap)
  ...
  (parameterize ([current-custodian cust])
    ...
    (define (ret post+ [first? #f])
      ; Finitize the new post condition and add it to current finitization map.
      (define list-post+
        (if (list? post+) post+ (list post+)))
      (when (not first?)
        (finitize list-post+ (current-bitwidth) fin-map))

      ; guess the first candidate (since we have no concrete inputs)
      (solver-assert guesser
                     (if first?
                       list-post+
                       (map (curry hash-ref fin-map) list-post+)))
