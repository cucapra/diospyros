...
void kernel(float* input_A, float* input_B, float* input_C);
 ...
 // Naive
 void naive_matrix_sub(float *a, float *b, float *c, int row1, int col1, int col2) {
   for (int y = 0; y < row1; y++) {
     for (int x = 0; x < col2; x++) {
      // c[col2 * y + x] = 0;
      // for (int k = 0; k < col1; k++) {
         c[col2 * y + x] = a[col1 * y + x] - b[col2 * y + x];
       }
     }
   }
 }
 // Naive hard-coded size
 void naive_matrix_sub_hard_size(float *a, float *b, float *c) {
   for (int y = 0; y < A_ROWS; y++) {
     for (int x = 0; x < B_COLS; x++) {
      // c[B_COLS * y + x] = 0;
      // for (int k = 0; k < A_COLS; k++) {
         c[B_COLS * y + x] = a[A_COLS * y + x] - b[B_COLS * y + x];
       }
     }
   }
 }
...
//  int P, float32_t *z);
 ...
 int main(int argc, char **argv) {
  ...
  naive_matrix_sub(a, b, c_spec,  A_ROWS, A_COLS, B_COLS);

  int time = 0;

  // Naive
  start_cycle_timing;
  naive_matrix_sub(a, b, c,  A_ROWS, A_COLS, B_COLS);
  stop_cycle_timing;
  time = get_time();
  ...
  // Naive, hard-coded size
  start_cycle_timing;
  naive_matrix_sub_hard_size(a, b, c);
  stop_cycle_timing;
  time = get_time();
  print_matrix(c, A_ROWS, B_COLS);
  output_check(c, c_spec, A_ROWS, B_COLS);
