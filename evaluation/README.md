...
(10×10 by 10×10 `MatMul`) over a range of equality saturation timeouts, demonstrating that
cycle-level performance improves with tunable, longer compilation times.
...
2. **Evaluation on licensed instruction set simulator (ISS).**
  Our compiler targets the [Tensilica Fusion G3][fusiong3], which does not have an
...
[fusiong3]: https://ip.cadence.com/ipportfolio/tensilica-ip/fusion
